...
export class Register {
  ...
  form_menu() {
    ...
    for (let i = 0; i < this.container.size; i++) {
      ...
      if (!item) continue;
      form.button(
        `${i} ${item.nameTag ? item.nameTag.split("\n")[0] : item.typeId.replace(/^.*?:/, "")}`,
      );
      indices.push(i);
    }
    form.button("§l註冊頁面");
  ...
  #form_edit(idx: number) {
    ...
    const name = item.nameTag
      ? Register.#parse(item.nameTag.replace(/^§r/, ""))
      : "";
    ...
    const form = new ModalFormData()
      ...
      .textField(
        '§l指令("/"換行, toPage:頁面名稱 可切換頁面, closeUI 關閉UI)',
        "",
        { defaultValue: processedCommands },
      );
  ...
  static load() {
    ...
    for (const pageName in pages) {
      ...
      ChestUI.setUIPage(
        pageName,
        new Page(btnWithIdx, { size: jsonPage.size }),
      );
    }
  }

  static #buildButton(jsonButton: JsonButton) {
    const { name, typeId, amount, lore, commands, clickSound, toPage } =
      jsonButton;
    const processedCommands = commands
      ? this.#split(commands).map((command) => command.trim())
      : [];
  ...
  static #setPages(pages: { [key: string]: JsonPage }) {
    world.setDynamicProperty(
      "yb:eui_pages",
      JSON.stringify(pages) || undefined,
    );
  }
