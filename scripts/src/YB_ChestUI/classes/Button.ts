...
export class Button {
  ...
  constructor(nameTag: string, itemType: string, options: buttonOptions = {}) {
  ...
  getItem() {
    return ChestUI.newUIItem(this.nameTag, this.itemType, {
      amount: this.amount,
      lore: this.lore,
    });
  }
  ...
  updateCheck(item: ItemStack, buttonItem = this.getItem()) {
    switch (this.updateType ?? ChestUI.config.defaultButtonUpdateType) {
      ...
      case UpdateType.amount:
        return item?.amount !== buttonItem.amount;
      case UpdateType.typeId:
        return item?.typeId !== buttonItem.typeId;
      case UpdateType.stackable:
        return !item?.isStackableWith(buttonItem);
      default:
        return false;
    }
  }
